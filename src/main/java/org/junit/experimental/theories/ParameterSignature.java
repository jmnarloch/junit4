...
public class ParameterSignature {
    ...
    private boolean isAssignableViaTypeConversion(Class<?> targetType, Class<?> candidate) {
        if (targetType.isPrimitive()) {
            return canBeBoxedTo(candidate, targetType);
        } else {
            return     targetType.isAssignableFrom(candidate)
            || (canBeBoxedTo(candidate, targetType));
        }
    }
    ...
    private <T extends Annotation> T findDeepAnnotation(
        ...
        for (Annotation each : annotations) {
            ...
            each.annotationType().getAnnotations(), annotationType, depth - 1);
            if (candidate != null) {
