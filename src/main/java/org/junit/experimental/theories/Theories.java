...
public class Theories extends BlockJUnit4ClassRunner {
    ...
    private void validateDataPointFields(List<Throwable> errors) {
        ...
        for (Field field : fields) {
            ...
            if (!Modifier.isStatic(field.getModifiers())) {
                errors.add(new Exception("DataPoint field " + field.getName() + " must be static"));
            }
            if (!Modifier.isPublic(field.getModifiers())) {
                errors.add(new Exception("DataPoint field " + field.getName() + " must be public"));
            }
        }
    }
    ...
    private void validateDataPointMethods(List<Throwable> errors) {
        ...
        for (Method method : methods) {
            ...
            if (!Modifier.isStatic(method.getModifiers())) {
                errors.add(new Exception("DataPoint method " + method.getName() + " must be static"));
            }
            if (!Modifier.isPublic(method.getModifiers())) {
                errors.add(new Exception("DataPoint method " + method.getName() + " must be public"));
            }
        }
    }
    ...
    private void validateParameterSupplier(Class<? extends ParameterSupplier> supplierClass, List<Throwable> errors) {
        Constructor<?>[] constructors = supplierClass.getDeclaredConstructors();
        
        if (constructors.length != 1) {
            errors.add(new Exception("ParameterSupplier " + supplierClass.getName() + " must have only one constructor (either empty or taking only a TestClass)"));
        } else {
            Class<?>[] parameterTypes = constructors[0].getParameterTypes();
            
            if (!(parameterTypes.length == 0) && !parameterTypes[0].equals(TestClass.class)) {
                errors.add(new Exception("ParameterSupplier " + supplierClass.getName() + " constructor should take either nothing or a single TestClass instance"));
            }
        }
    }
    ...
    public static class TheoryAnchor extends Statement {
        ...
        public void evaluate() throws Throwable {
            runWithAssignment(new Assignments());
            if (successes == 0) {
        ...
        protected void runWithAssignment(Assignments parameterAssignment)
                throws Throwable {
            if (!nullsOk()) {
                parameterAssignment.assertNoNullCause();
            }
        ...
        protected void runWithIncompleteAssignment(Assignments incomplete)
            ...
            if (nullsOk()) {
                runWithCompleteAssignment(incomplete);
            }
        }
        ...
        protected void runWithCompleteAssignment(final Assignments complete)
                throws Throwable {
            if (nullsOk()) {
                runWithAssignment(complete);
            } else {
                runWithAssignment(complete);  // runs the test
                if (successes == 0) {
                    Assert.fail("Never found parameters that satisfied method assumptions.  Violated assumptions: "
                    + fInvalidParameters);
                }
        ...
        private Statement methodCompletesWithParameters(
            ...
            return new Statement() {
                @Override
                public void evaluate() throws Throwable {
                    method.invokeExplosively(freshInstance, complete.getMethodArguments());
                }
            };
        }
        ...
        protected void reportParameterizedError(Throwable e, Object... params)
                throws Throwable {
            new ParameterizedAssertionError(e, testMethod.getName(), params).getTestRule()
            .evaluate();
        }

        private boolean nullsOk() {
            return testClass.getAnnotation(Theory.class).nullsAccepted();
        }
