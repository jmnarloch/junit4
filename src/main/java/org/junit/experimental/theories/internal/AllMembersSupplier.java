...
public class AllMembersSupplier extends ParameterSupplier {
    ...
    private void addMultiPointMethods(ParameterSignature sig, List<PotentialAssignment> list) throws Throwable {
        for (FrameworkMethod dataPointsMethod : getDataPointsMethods(sig)) {
            Class<?>[] methodParameters = dataPointsMethod.getMethod().getParameterTypes();
            Class<?> dataType = methodParameters[methodParameters.length - 1];
            
            if (sig.canPotentiallyAcceptType(dataType)) {
                if (dataType.isArray()) {
                    Object array = dataPointsMethod.invokeExplosively(null, sig.getParameterTypes());
                    addArrayValues(sig, dataPointsMethod.getName(), list, array);
                } else if (Iterable.class.isAssignableFrom(dataType)) {
                    Iterable<?> iterable = (Iterable<?>) dataPointsMethod.invokeExplosively(null, sig.getParameterTypes());
                    addIterableValues(sig, dataPointsMethod.getName(), list, iterable);
                }
            }
        }
    }
    ...
    private void addDataPointsValues(Class<?> type, ParameterSignature sig, String name, 
        ...
        addIterableValues(sig, name, list, value);
    }
    ...
    private Object getStaticFieldValue(final Field field) {
        try {
            ...
            "unexpected: field from getClass doesn't work for null");
